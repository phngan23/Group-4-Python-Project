...
class PomodoroTimer {
    ...
    setupEventListeners() {
        ...
        if (toggleBtn) {
            ...
            console.warn("Không tìm thấy nút có id='toggle-btn'. Hãy kiểm tra lại file HTML.");
        }
    ...
    start() {
        ...
        if (this.currentMode === "pomodoro" && (!subjectSelect.value || subjectSelect.value === "" || subjectSelect.value === "new")) {
    ...
    showConfirmationModal() {
        ...
        document.querySelectorAll('button:not(.confirm-btn):not(.cancel-btn)')
    ...
    async stopStudySessionAPI() {
        try {
            ...
            if (data.status === "stopped") {
                alert(`Bạn đã học ${Math.round(data.duration_seconds / 60)} phút và nhận ${data.points_awarded} coins!`);
            }
