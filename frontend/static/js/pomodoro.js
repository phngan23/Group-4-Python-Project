...
class StudyAPI {
  ...
  static start(subjectId) {
    return this.request("/study/api/start/", "POST", { subject_id: subjectId });
  }
  static stop() {
    return this.request("/study/api/stop/", "POST");
  } // Lưu và kết thúc
  static cancel(sessionId) {
    return this.request("/study/api/cancel/", "POST", {
      session_id: sessionId,
    });
  } // Xóa bỏ
  static pause() {
    return this.request("/study/api/pause/", "POST");
  }
  static resume() {
    return this.request("/study/api/resume/", "POST");
  }
...
class PomodoroTimer {
  ...
  restoreState(state) {
    ...
    this.updateModeUI();
    this.updateUIStatus(
      this.isRunning ? (this.isPaused ? "paused" : "running") : "idle"
    );
  }
  ...
  async handleResetRequest() {
    if (this.currentMode === "pomodoro") {
      ...
      if (this.studiedSeconds >= 60) {
        ...
        document.getElementById("studied-time").textContent = Math.floor(
          this.studiedSeconds / 60
        );
      }
  ...
  updateModeUI() {
    ...
    document.getElementById("current-session-type").textContent =
  ...
  setupEventListeners() {
    ...
    document.querySelectorAll(".preset-btn").forEach((btn) => {
      btn.addEventListener("click", (e) =>
        this.setTime(parseInt(e.target.dataset.minutes))
      );
    });

    // 5. Nút set thời gian Custom
    const setCustomBtn = document.getElementById("set-custom-time");
    const customInput = document.getElementById("custom-minutes"); // Lấy thẻ input
    ...
    if (setCustomBtn && customInput) {
      ...
      const handleCustomTime = () => {
        ...
        if (val && parseInt(val) > 0) {
          ...
          this.setTime(parseInt(val));
          customInput.value = ""; // Xóa chữ trong ô input sau khi set xong
        }
      };

      // 1. Sự kiện Click nút "Set Time"
      setCustomBtn.addEventListener("click", handleCustomTime);

      // 2. Sự kiện nhấn phím Enter trên ô input
  ...
  setupEmotionModal() {
    ...
    document
      .getElementById("emotion-skip-btn")
      ?.addEventListener("click", closeEmotion);
  }
}
