...
function showPredictionResult(data) {
    ...
    const predictionTip = document.getElementById('prediction-tip') ||
        ...
        return `
            <div class="task-item" data-task-id="${task.id}">
                ...
                <div class="task-content">
                    ...
                    <div class="task-meta">
                        <span class="task-category">${categoryIcon} ${getCategoryDisplay(task.category)}</span>
                        <span class="${priorityClass}">${getPriorityIcon(task.priority)} ${task.priority.toUpperCase()}</span>
                        ...
                        ${task.due_date ? `
                            <span class="${isOverdue ? 'task-overdue' : ''}">
                                ...
                                ${!isCompleted ? `<small>(${task.time_left})</small>` : ''}
                            </span>
                        ` : ''}
                    </div>
                </div>
    ...
    .then(data => {
        if (data.status === 'success') {
            if (data.new_status === 'completed') {
                showNotification('Task completed! Coins have been added to your account.', 'success');
            }
...
function trainAIModelFromList() {
    ...
    if (resultDiv) {
        resultDiv.innerHTML = '<i class="fas fa-sync fa-spin"></i> AI is learning from your completed tasks...';
    }
