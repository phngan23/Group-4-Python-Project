...
class PomodoroTimer {
    constructor() {
        ...
        this.init(); // Bắt đầu mọi thứ
    }

    /* ========== INIT ========== */
    init() {
        this.setupEventListeners(); // Thiết lập event handlers
    ...
    // Lưu trạng thái timer vào localStorage để khôi phục khi reload trang
    // Giúp user không mất session khi vô tình tắt trang
    saveState() {
        const state = {
    ...
    async finishStudying() {
        ...
        try {
            ...
            const actualMinutes = Math.round(data.duration_seconds / 60); 
