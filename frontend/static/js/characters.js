...
function renderCharactersGrid() {
    ...
    container.innerHTML = characters.map(character => {
        ...
        const statusClass = character.is_unlocked ? 'status-unlocked' : 'status-locked';
        const statusText = character.is_unlocked ? 'UNLOCKED' : 'LOCKED';
        
        return `
            <div class="character-card ${isActive ? 'active' : ''}" 
                 onclick="${character.is_unlocked ? `selectCharacter(${character.id})` : ''}">
                ...
                <div class="character-rarity ${character.rarity}">${character.rarity.toUpperCase()}</div>
                ...
                ` : `
                    <button class="select-btn" onclick="event.stopPropagation(); selectCharacter(${character.id})"
                            ${isActive ? 'disabled' : ''}>
